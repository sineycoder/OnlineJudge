...
public class EcoderCommonController {
    ...
    public Map<String,Object> verifyLogin(HttpSession session) throws Exception {
    ...
    public Map<String,Object> getProblems(@PathVariable("page")int page,HttpSession session) throws Exception {
        ...
        records.setProblemIsShow(true);
        pagination.setTotal(commonService.queryProblemsCounts(records));
    ...
    public Map<String,Object> getContestResult(@PathVariable("page")int page)throws Exception{
        ...
        Pagination pagination = new Pagination();
        pagination.setCurrentPage(page);
        pagination.setTotal(commonService.queryContestTotals(true));
    ...
    public String uploadImage(@RequestParam("img") MultipartFile file,HttpServletRequest request) throws IOException {
        if(file.getSize() < 1024 * 1024 * 5){
            String url = request.getRequestURL().toString();
            url = url.substring(0,url.lastIndexOf("/"));
            String filename = UUID.randomUUID().toString()
                    .replace("-","")+
                    tools.getFilenameSuffix(file.getOriginalFilename());
            file.transferTo(new File(Tools.IMAGE_UPLOAD+"/"+filename));
            return url+"/showImg?img="+filename;
        }
        return null;
    }

    @RequestMapping("/showImg")
    public void showImg(@RequestParam("img")String imgName,HttpServletResponse response) throws Exception {
        tools.writeImage(response,Tools.IMAGE_UPLOAD+"/"+imgName);
